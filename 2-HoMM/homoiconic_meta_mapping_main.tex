...
\end{subfigure}
\caption[Performing and transforming tasks with the HoMM architecture.]{Performing and transforming tasks with the HoMM architecture.} \label{fig:HoMM_architecture}
\end{figure}

We propose Homoiconic Meta-Mapping (HoMM) architectures as an implementation of a system that can perform tasks, and adapt to task alterations via meta-mappings. In this section, we describe the architecture and training process.
...
\textbf{Task domains:} The meta-mapping approach is very general. Because the assumptions are simply that the basic tasks are mappings from inputs to outputs, and that meta-mappings transform basic tasks, the approach can be applied to almost any task domain. We demonstrate our method across polynomial regression, card games, visual classification, and reinforcement learning (RL). We are required to make a slight alteration only in how examples are processed in RL, because RL algorithms receive rewards as feedback for a single action they take in each state, rather than receiving a supervisory signal on all outputs. We thus replace the (input, target) examples used to infer a supervised task with (state, (action, reward)) examples, which are the basic currency of RL algorithms. The rest of the approach is identical across tasks, although the network architectures vary depending on the types of input and output (for example, we use CNNs to process the visual input in the visual classification and RL domains). See the suppmental material for full architecture specifications. 

\section{Experiments}

Meta-mapping is an extremely general framework, that can be applied to most paradigms of machine-learning with minor modifications. We demonstrate our results in four settings, ranging from regression to classification to reinforcement learning. We summarize the contributions of the different experiments in Table \ref{table:HoMM_experiment_summary}. 

\begin{table}
\centering
\begin{tabular}{|p{2.4cm}|p{2.8cm}ccp{1.8cm}p{1.9cm}|}
\hline
\textbf{Experiment} & \textbf{Motivation} & \begin{minipage}[t]{2.7cm}\centering\textbf{Held-out meta-mappings}\end{minipage} & \begin{minipage}[t]{2.2cm}\centering\textbf{Language comparison}\end{minipage} & \textbf{Paradigm} & \textbf{Input}\\[1.3em]
\hline
Polynomials & Proof of concept & \checkmark & & Regression & Vector (\(\mathbb{R}^4\))\\[0.5em] 
Cards & Card games and comparison to humans & & \checkmark & Regression & Several-hot vector\\[0.5em]
Visual concepts & Cognitive and computer vision relevance & \checkmark & \checkmark & Classification & \(50 \times 50\) RGB image\\[0.5em]
Reinforcement learning & Cognitive and AI relevance & & \checkmark & RL & \(91 \times 91\) RGB image\\[0.5em]

\hline
\end{tabular}

\caption{The contributions of our four sets of experiments. Our results span various computational paradigms and various degrees of input complexity, and are motivated by both cognitive and AI relevance.} \label{table:HoMM_experiment_summary}
\end{table}
...
\end{scope}
\end{tikzpicture}
\caption{Polynomial tasks and meta-mappings}\label{fig:HoMM_polynomials:tasks}
\end{subfigure}%
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics[width=\textwidth]{2-HoMM/figures/polynomials_adaptation.png}
\caption{Meta-mapping results.}\label{fig:HoMM_polynomials:results}
\end{subfigure}
\caption{The polynomial task domain and results. (\subref{fig:HoMM_polynomials:tasks}) A basic polynomial task consists of regressing a single polynomial, i.e. the inputs are points in \(\mathbb{R}^4\) and the outputs are the value of the polynomial at that point. These basic regression tasks can be transformed by various meta-mappings, such as multiplying by a constant, or permuting their variables. (\subref{fig:HoMM_polynomials:results}) Results of applying our HoMM architecture in this setting. We plot zero-shot performance (normalized, see text) on new tasks via meta-mappings. The system not only-generalizes trained meta-mappings to examples it has never seen before (purple), but also generalizes to held-out meta-mappings from examples (orange), and does both substantially better than a baseline model which does not adapt (dotted lines). Thus our approach is able to flexibly adapt to a new polynomial without any data from that polynomial, based on that polynomial's relationship to polynomials it has experienced.}\label{fig:HoMM_polynomials}
\end{figure}
...
The context we give the model is in the form of examples of the task as (input, output) pairs as usual. These are used to construct a task representation, which is then used to modulate the parameters in the task network, via a HyperNetwork. We trained the model repeatedly with different proportions of training on the word task vs. the color task, in order to investigate the default vs. controlled behavior in different training regimes. Specifically, we compared training the model to the point that it barely mastered the less frequent task (when it first achieves 100\% performance and cross-entropy loss \(< 0.3\) on both tasks) to the point that it had mastered both tasks (100\% performance and cross-entropy loss \(<0.01\) on both). We then tested the model's default behavior by giving it an all-zeros task representation, and seeing whether its performance was more aligned with the ``word'' or ``color'' task.
...
In summary, our perspective builds on several lines of prior work in machine learning. While there has been substantial prior work on meta-learning, task representation, and there have even been other approaches to zero-shot task performance, to the best of our knowledge none of the prior work has explored zero-shot performance of a task via meta-mappings. We showed experimentally that this approach yields better performance than alternative approaches across a variety of domains, and suggest that it may complement other tools, such as model-based RL.
...
Some aspects of the HoMM architecture may also seem reminiscent of the modularity of mind for which Fodor advocated \citep{Fodor1983modularity}, particular the fact that we divided the model into feed-forward input and output systems, with the flexible, task-specific computations in the middle shared across many domains. In fact, we believe that perception and task-processing are mutually constraining and reinforcing, but for simplicity our model does not incorporate all aspects of this. Cognitive modeling always requires some simplification in order to provide a useful description of the system. (Again, see Chapter \ref{chapter:conclusions} for further discussion.)   
